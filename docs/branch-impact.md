...
1. **Create Plan**: `pprof.branch_impact.plan` - Generate execution plan
2. **Review**: Examine steps and estimated time
3. **Execute**: `pprof.branch_impact.execute` - Run the plan (walk away!)
...
// This calls:
mcp__pprof__pprof_branch_impact_plan({
  service: "ratelimit",
...
mcp__pprof__pprof_branch_impact_execute({
...
```typescript
mcp__pprof__pprof_branch_impact({
  service: "ratelimit",
  out_dir: "/tmp/profile-comparison"
})
```
...
- Stashed with timestamped message: `pprof_branch_impact auto-stash <timestamp>`
...
After running `pprof.branch_impact`, analyze the results:
...
# Find auto-stash
git stash list | grep "pprof_branch_impact"
...
```typescript
// vs main
const mainComparison = await pprof_branch_impact({
  service: "ratelimit",
...
// vs production
const prodComparison = await pprof_branch_impact({
  service: "ratelimit",
...
const result = await mcp__pprof__pprof_branch_impact({
  service: "ratelimit",
